...
**coldforge** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
the suite's integration SDK. It maps any tool's JSON into a canonical **Finding** and
forwards it to the platforms that fit the **Identity / persona** domain.
...
Once `coldforge` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
coldforge ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
coldforge ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
coldforge ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `coldforge` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="coldforge") for rec in records]   # records = your JSON output
