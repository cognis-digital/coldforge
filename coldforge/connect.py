"""Native cognis-connect emit for coldforge — forward findings to any platform.

Maps coldforge's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    coldforge ... --format json | coldforge-emit --to stix
    coldforge-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop("source", None)
        out["title"] = out.get("summary", "")
        out["severity"] = out.get("confidence", "info")
        out["type"] = out.get("category", "")
        out["description"] = out.get("details", "")
        out["tags"] = out.get("tags", []).split(",") if "tags" in out else []
